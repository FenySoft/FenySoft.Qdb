...
{
    //public class DirectoryHeap : ITHeap
    //{
    //    private TAtomicFile systemFile;
    ...
    //        systemFile = new TAtomicFile(Path.Combine(directory, "system.stsdb4"));
    ...
    //    {
    //        return new TOptimizedFileStream(GetFileName(handle, version), FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite, size);
    //    }

    //    private Stream OpenFile(long handle, int version, int size)
    //    {
    //        return new TOptimizedFileStream(GetFileName(handle, version), FileMode.Open, FileAccess.Read, FileShare.Read, size);
    //    }
