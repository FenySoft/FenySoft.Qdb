namespace FenySoft.Qdb.Storage
{
    ...
    public class TSpace
    {
        ...
        private List<TPtr> free = new List<TPtr>(); //free chunks are always: ordered by position, not overlapped & not contiguous
        ...
        public void Add(TPtr freeChunk)
        {
            if (free.Count == 0)
                free.Add(freeChunk);
            else
        ...
        public TPtr Alloc(long size)
        {
            ...
            TPtr ptr = free[activeChunkIndex];
            ...
            return new TPtr(pos, size);
        }

        public void Free(TPtr ptr)
        {
        ...
        public void Deserealize(BinaryReader reader)
        {
            ...
            {
                var ptr = TPtr.Deserialize(reader);
                free.Add(ptr);
                FreeBytes += ptr.Size;
            }
        }
