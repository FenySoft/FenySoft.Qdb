...
namespace FenySoft.Qdb.Storage
{
    public class THeap : ITHeap
    {
        ...
        private TAtomicHeader header;
        private readonly TSpace space;

        //updated every time after Serialize() invocation.
        private long maxPositionPlusSize;

        //handle -> pointer
        private readonly Dictionary<long, TPointer> used;
        private readonly Dictionary<long, TPointer> reserved;
        ...
        public THeap(Stream stream, bool useCompression = false, AllocationStrategy strategy = AllocationStrategy.FromTheCurrentBlock)
        {
            ...
            used = new Dictionary<long, TPointer>();
            reserved = new Dictionary<long, TPointer>();

            if (stream.Length < TAtomicHeader.SIZE) //create new
            {
                header = new TAtomicHeader();
                header.UseCompression = useCompression;
                space.Add(new TPtr(TAtomicHeader.SIZE, long.MaxValue - TAtomicHeader.SIZE));
            }
            else //open exist (ignore the useCompression flag)
            {
                header = TAtomicHeader.Deserialize(Stream);
        ...
        public THeap(string fileName, bool useCompression = false, AllocationStrategy strategy = AllocationStrategy.FromTheCurrentBlock)
            :this(new TOptimizedFileStream(fileName, FileMode.OpenOrCreate), useCompression, strategy)
        {
        }
        ...
        {
            maxPositionPlusSize = TAtomicHeader.SIZE;
        ...
        private void Deserialize(BinaryReader reader)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var pointer = TPointer.Deserialize(reader);
        ...
        public void Write(long handle, byte[] buffer, int index, int count)
        {
            ...
            lock (SyncRoot)
            {
                ...
                TPtr ptr = space.Alloc(size);
                used[handle] = pointer = new TPointer(currentVersion, ptr);
        ...
        public void Commit()
        {
            lock (SyncRoot)
            {
                ...
                {
                    if (header.SystemData != TPtr.NULL)
                        space.Free(header.SystemData);

                    Serialize(new BinaryWriter(ms));

                    TPtr ptr = space.Alloc(ms.Length);
        ...
        public KeyValuePair<long, TPtr>[] GetUsedSpace()
        {
            lock (SyncRoot)
            {
                KeyValuePair<long, TPtr>[] array = new KeyValuePair<long, TPtr>[used.Count + reserved.Count];
