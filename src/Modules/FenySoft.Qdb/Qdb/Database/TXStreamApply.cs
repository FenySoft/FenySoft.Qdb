...
{
    public class TXStreamApply : ITApply
    {
        const int BLOCK_SIZE = TXStream.BLOCK_SIZE;

        public TXStreamApply(TLocator path)
        {
            Locator = path;
        }
            ...
            foreach (var opr in operations)
            {
                switch (opr.Code)
                {
                    case TOperationCode.REPLACE:
                        {
                            if (Replace(data, (TReplaceOperation)opr))
                                isModified = true;
                        }
                        break;
                    case TOperationCode.DELETE:
                        {
                            TDeleteRangeOperation o = new TDeleteRangeOperation(opr.FromKey, opr.FromKey);
                            if (Delete(data, (TDeleteRangeOperation)o))
                                isModified = true;
                        }
                        break;
                    case TOperationCode.DELETE_RANGE:
                        {
                            if (Delete(data, (TDeleteRangeOperation)opr))
                                isModified = true;
                        }
                        break;
        ...
        private bool Replace(ITOrderedSet<ITData, ITData> set, TReplaceOperation operation)
        {
            Debug.Assert(operation.Scope == TOperationScope.Point);
        ...
        private bool Delete(ITOrderedSet<ITData, ITData> set, TDeleteRangeOperation operation)
        {
            long from = ((TData<long>)operation.FromKey).Value;
