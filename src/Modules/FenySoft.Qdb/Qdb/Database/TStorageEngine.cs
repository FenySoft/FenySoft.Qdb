...
{
    public class TStorageEngine : TWTree, ITStorageEngine
    {
        //user scheme
        ...
        public TStorageEngine(ITHeap heap)
            : base(heap)
        {
        ...
        private Item1 Obtain(string name, int structureType, TDataType keyDataType, TDataType recordDataType, Type keyType, Type recordType)
        {
            ...
            if (!map.TryGetValue(name, out item))
            {
                ...
                TXTablePortable table = new TXTablePortable(this, locator);
            ...
            if (item.Table == null)
                item.Table = new TXTablePortable(this, item.Locator);
        ...
        public ITTable<ITData, ITData> OpenXTablePortable(string AName, TDataType AKeyDataType, TDataType ARecordDataType)
        {
            lock (SyncRoot)
            {
                var item = Obtain(AName, TStructureType.XTABLE, AKeyDataType, ARecordDataType, null, null);
        ...
        public ITTable<TKey, TRecord> OpenXTablePortable<TKey, TRecord>(string AName, TDataType AKeyDataType, TDataType ARecordDataType, ITTransformer<TKey, ITData> AKeyTransformer, ITTransformer<TRecord, ITData> ARecordTransformer)
        {
            lock (SyncRoot)
            {
                ...
                if (item.Portable == null)
                    item.Portable = new TXTablePortable<TKey, TRecord>(item.Table, AKeyTransformer, ARecordTransformer);
        ...
        public ITTable<TKey, TRecord> OpenXTable<TKey, TRecord>(string AName)
        {
            lock (SyncRoot)
            {
                ...
                var item = Obtain(AName, TStructureType.XTABLE, keyDataType, recordDataType, keyType, recordType);

                if (item.Direct == null)
                    item.Direct = new TXTable<TKey, TRecord>(item.Table);

                return (TXTable<TKey, TRecord>)item.Direct;
            }
        }
        ...
        public TXFile OpenXFile(string AName)
        {
            lock (SyncRoot)
            {
                var item = Obtain(AName, TStructureType.XFILE, TDataType.Int64, TDataType.ByteArray, typeof(long), typeof(byte[]));

                if (item.File == null)
                    item.File = new TXFile(item.Table);
        ...
        private class Item1
        {
            ...
            public TXTablePortable Table;

            public ITable Direct;
            public ITable Portable;
            public TXFile File;            

            public Item1(TLocator locator, TXTablePortable table)
            {
                Locator = locator;
                Table = table;
            }
