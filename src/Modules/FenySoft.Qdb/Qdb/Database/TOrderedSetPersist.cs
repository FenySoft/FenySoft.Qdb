...
{
    public class TOrderedSetPersist : ITPersist<ITOrderedSet<ITData, ITData>>
    {
        ...
        public TOrderedSetPersist(ITIndexerPersist<ITData> keyIndexerPersist, ITIndexerPersist<ITData> recordIndexerPersist, ITOrderedSetFactory orderedSetFactory)
        {
        ...
        public TOrderedSetPersist(ITPersist<ITData> keyPersist, ITPersist<ITData> recordPersist, ITOrderedSetFactory orderedSetFactory)
        {
        ...
        private void WriteVertical(BinaryWriter writer, ITOrderedSet<ITData, ITData> data)
        {
            ...
            lock (data)
            {
                ...
                TCountCompression.Serialize(writer, checked((ulong)rows.Length));
                writer.Write(data.IsInternallyOrdered);
            }
            ...
            foreach (var stream in streams)
            {
                using (stream)
                {
                    TCountCompression.Serialize(writer, checked((ulong)stream.Length));
                    writer.Write(stream.GetBuffer(), 0, (int)stream.Length);
                }
            }
        }

        private static readonly TKeyValuePairHelper<ITData, ITData> helper = new TKeyValuePairHelper<ITData, ITData>();
